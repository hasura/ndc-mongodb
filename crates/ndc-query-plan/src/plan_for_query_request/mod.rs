...
use crate::{self as plan, QueryPlan};
...
use self::{
    ...
    with_name::WithNameRef,
};

type Result<T> = std::result::Result<T, QueryPlanError>;

#[derive(Clone, Debug)]
pub struct QueryContext<'a, ScalarType>
where
    ScalarType: Clone,
{
    ...
    pub object_types: Cow<'a, BTreeMap<String, plan::ObjectType<ScalarType>>>,
    pub scalar_types: Cow<'a, BTreeMap<String, ndc::ScalarType>>,
}

impl<S: Clone> QueryContext<'_, S> {
    ...
    pub fn find_collection_object_type(
        &self,
        collection_name: &str,
    ) -> Result<WithNameRef<plan::ObjectType<S>>> {
    ...
    pub fn find_object_type<'a>(
        ...
        let object_type = self
            ...
            .ok_or_else(|| QueryPlanError::UnknownObjectType(object_type_name.to_string()))?;
...
fn find_object_field<'a, S>(
    object_type: &'a WithNameRef<plan::ObjectType<S>>,
    field_name: &str,
