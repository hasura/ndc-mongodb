...
use crate::{
    ...
    ConnectorTypes, Query, QueryContext, QueryPlanError, Relationship,
};
...
pub struct QueryPlanState<'a, T: QueryContext> {
    query_context: &'a T,
    ndc_relationships: &'a BTreeMap<String, ndc::Relationship>,
...
impl<T: QueryContext> QueryPlanState<'_, T> {
    pub fn new<'a>(
        ...
        QueryPlanState {
            query_context,
            ndc_relationships: collection_relationships,
    ...
    pub fn state_for_subquery<'a>(&'a self) -> QueryPlanState<'a, T> {
        QueryPlanState {
            ...
            ndc_relationships: self.ndc_relationships,
            relationships: Default::default(),
    ...
    pub fn register_relationship<'a>(
        ...
        let ndc_relationship = lookup_relationship(self.ndc_relationships, &ndc_relationship_name)?;
    ...
    /// Use this with the top-level plan to get both relationships, and unrelated joins.
