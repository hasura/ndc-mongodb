...
use super::{helpers::lookup_relationship, ConversionError};
...
) -> impl Iterator<Item = Result<TraversalStep, ConversionError>> {
    let QueryRequest {
        collection,
...
fn query_traversal_helper<'a>(
    context: Context<'a>,
    query: &'a Query,
) -> impl Iterator<Item = Result<TraversalStep<'a, 'a>, ConversionError>> {
...
fn field_relationship_traversal<'a>(
    context: Context<'a>,
    field: &'a Field,
) -> Box<dyn Iterator<Item = Result<TraversalStep<'a, 'a>, ConversionError>> + 'a> {
    match field {
...
where
    ...
    I: Iterator<Item = Result<TraversalStep<'a, 'a>, ConversionError>>,
{
    ast_nodes
        .into_iter()
