...
fn serialize_row_set_aggregates_only(
    ...
    Ok(RowSet {
        aggregates: Some(serialize_aggregates(options, path, aggregates, doc)?),
        rows: None,
        groups: None,
    })
}
...
fn serialize_row_set_groups_only(
    ...
    Ok(RowSet {
        aggregates: None,
        rows: None,
        groups: Some(serialize_groups(options, path, grouping, docs)?),
    })
}
...
fn serialize_row_set(
    ...
    let aggregates = query
        ...
        .map(|aggregates| {
            ...
            serialize_aggregates(options, path, aggregates, aggregate_values)
        })
        .transpose()?;
    ...
    let groups = query
        .groups
        .as_ref()
        .map(|grouping| serialize_groups(options, path, grouping, row_set.groups))
        .transpose()?;
    ...
    let aggregate_values = query_aggregates
        ...
        .map(|(key, aggregate)| {
            let json_value = match value.get(key.as_str()).cloned() {
                Some(bson_value) => bson_to_json(
                    options.extended_json_mode,
                    &type_for_aggregate(aggregate),
                    bson_value,
                )?,
                None => {
...
fn serialize_groups(
    ...
    docs.into_iter()
        .map(|doc| {
            ...
            let aggregates = serialize_aggregates(options, path, &grouping.aggregates, doc)?;
